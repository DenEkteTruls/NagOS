#include "kernel.h"
#include "bin/devices/keyboard.h"

// if the writing is active
int writing = 0;
...
void print_string(char *str, int newLine)
{
  uint32 index = 0;
  while(str[index]){
...
void print_int(int num, int newLine)
{
  ...
  itoa(num, str_num);
  if(newLine) {
    print_string(str_num, 1);
  } else {
    print_string(str_num, 0);
  }
}
...
void kernel_entry()
{
  ...
  */
  print_string("Welcome to NagOS", 1);
  print_string("Booting...",0);
  sleep(0x4FFFFF);
  clear_vga_buffer(&vga_buffer, g_fore_color, g_back_color);
